...
int parse(char haystack[], char domain[], char port[], char path[]) {
	...
	//sending and receiving the file in packets of chunks if it's an ftp request
	char *buff;
	int filesize = 0;
	//how's it going to know the size of file
	//can it actually start transferring or breaking down file in chunks if it gets an ftp request?
	//do we generate a response for each chunk? so that would mean putting the whole parse generating for response header within a loop
	//do we need to actually implement ftp? like does our program need to
	//be able to read an ftp request? can it just read it as a http request and take the file from the http request header
	//it's going to need a new socket for sure, does it need a new connection? since the server is already listening 
	//does it need to create a new socket for every packet, i dont think so
	//formulating the ftp request, i read something abt a server, using
	//control and data ports 
	
	if(needle=10) { 
	//set up ftp server and ftp transaction
	//Creating control socket to communicate with the ftp server 
	int csock = socket(AF_INET, SOCK_STREAM, 0);
	if(csock < 0) {
	error_out("Failed to create control socket");
	}
		
	// Create socket for control connection
	int csockfd;
	csockfd = create_socket();

	//Set server address and port
	//These will be casted to sockaddr (general-purpose socket struct) from sockaddr_in (IP-specific socketaddr struct)
	struct sockaddr_in serveraddr;
	serveraddr = ftp_control_connection(21);

	//Bind control socket to ftp address
	bind_wrap(csockfd,(struct sockaddr *) &serveraddr,sizeof(serveraddr));
	//return sockfd;                                                                         
	connect(csockfd, &serveraddr, sizeof(serveraddr));

	if(connect < 0) { 
	error_out("Failed to connect to control connection socket");
	}
	else {
	//initiate the ftp transaction 
	if(recv(csockfd, buff, 8192, 0) >= 1) { //wait for the server to start talking since we have connected
	if(strncmp(buff, "220", 3) == 0) { //whether ftp server responds to the client to know it's connected
	if(send(csockfd,"USER" + " " + "ra74", 9, 0) >= 1) { //sending login details
	if(recv(csockfd, buff, 8192, 0) >= 1) {
	if(strncmp(buff, "331", 3) == 0) {
	if(send(csockfd, "PASS" + " " + "1234", 9, 0) >= 1) {
	if(recv(csockfd, buff, 8192, 0) >= 1) {
	if(strncmp(buff, "230", 3) == 0) { //confirmation that you've logged on to the server
	if(send(csockfd, "PASV", "") >= 1) { //let server know you want to transfer a file 
	if(recv(csockfd, buff, 8192, 0) >= 1) { 
	if(strncmp(buff, "227" 3) == 0) {
	//need to split the return of the 227 command which would contain the ipaddr and port of the host server
	...
	int dsockfd;
	dsockfd = create_socket();
	
	//Set server address and port
	//These will be casted to sockaddr (general-purpose socket struct) from sockaddr_in (IP-specific socketaddr struct)
	struct sockaddr_in ftpaddr;
	ftpaddr = ftp_control_connection(ipadd, fport);
	                                                    
	//Bind control socket to ftp address
	bind_wrap(dsockfd,(struct ftpaddr *) &ftpaddr,sizeof(ftpaddr));
	//return sockfd;
	
	int fread=0; //files that it would read and write
	int filesize=0;

	connect(dsockfd, &ftpaddr, sizeof(ftpaddr)); 
	if(connect < 0) { 
	error_out("Failed to connect to data connection socket");
	}
	else { 
	if(send(csockfd, "STOR", 4, samplefile) >= 1) {
	do {
	fread = read(dsockfd, buff, MAXCHUNK);
	fsize= fsize + fread; //adds the no of bytes read to the filesize each time to obtain the file size
	if (fread > 0) { 
	write(samplefile, buff, fread);
	fsize = fsize + fread;  
	} while(!fileread)
	}
	
	if(recv(csockfd, buff, 8192, 0 >= 1)) {
	if(strncmp(buff, "150", 3)) { //know that the server got it all 
	close(dsockfd);
	if(send(csockfd, "QUIT", "") >= 1) {
	close(csockfd) } } } } } } } } } } } } } } } } }
...
struct sockaddr_in ftp_control_connection(int ipadd,int Controlport){
struct sockaddr_in serveraddr;
bzero(&serveraddr,sizeof(serveraddr));
serveraddr.sin_family = AF_INET; // set addr family
serveraddr.sin_addr.s_addr = htonl(INADDR_ANY); // bind to all available interfaces
serveraddr.sin_port = htons(Controlport); // sets port # to 21 for control connection port
return serveraddr;
}

struct sockaddr_in ftp_data_connection(int Dataport){
struct sockaddr_in serveraddr;
bzero(&serveraddr, sizeof(serveraddr));
serveraddr.sin_family = AF_INET; //set addr family
serveraddr.sin_addr.s_addr = htonl(INADDR_ANY); //binds to all available interfaces
serveraddr.sin_port = htons(Dataport); //sets port # to 20 for data connection port
return serveraddr;
}
